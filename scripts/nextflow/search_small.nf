...
params.calder_input_script = "${params.proj_dir}/scripts/processing/make_calder_input.py"
params.calder_parse_output = "${params.proj_dir}/scripts/processing/parse_calder_output.py"
...
process orchard {
    ...
    """
    /usr/bin/time -v python '${params.orchard_bin}' ${ssm} ${params_json} -k 10 -f 100 -n 16 results.npz 2>> timing.txt
    python '${params.pairtree_parse_output}' results.npz --output best_tree.txt     
    """
}
...
workflow {
    ...
    file("${params.outputDir}/orchard/").mkdirs()
