...
Notice that a feasible solution to this linear program is obtained
by setting
...
providing another proof of the sufficiency of the sum condition, and a lower
bound on the true solution that is easy to calculate.

\begin{theorem}
  Given a frequency vector $f \in \mathbb{R}^n$ and a clonal matrix $B \in \mathbb{R}^{n \times n}$,
  the minimum of 
  $$\lVert f^T - u^TB \rVert_1$$
  over all usage vectors $u \in \mathbb{R}^n$ is \emph{at least}
